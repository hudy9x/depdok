...
function Get-ReleaseVersion {
    ...
    Write-Host '  $Version = "0.28.0"; irm https://raw.githubusercontent.com/hudy9x/depdok-ladi/main/install.ps1 | iex' -ForegroundColor Yellow
    return $false
}

function Get-DownloadUrl {
    # Default to standard Tauri MSI installer name pattern for Windows
    $script:DownloadUrl = "https://github.com/$Repo/releases/download/v$($script:ReleaseVersion)/Depdok_$($script:ReleaseVersion)_x64_en-US.msi"
    $script:Filename = "Depdok_$($script:ReleaseVersion)_x64_en-US.msi"
...
if (-not (Get-ReleaseVersion)) { Wait-AndExit 1 }
Get-DownloadUrl
if (-not (Install-App)) { Wait-AndExit 1 }
if ($script:HasError) { Wait-AndExit 1 }
...
if ($Host.Name -eq "ConsoleHost") { Wait-AndExit 0 }
